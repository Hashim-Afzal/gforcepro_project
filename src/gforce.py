...
CMD_UUID = 'f000ffe1-0451-4000-b000-000000000000' # TI base 128 bit uuid 'f000XXXX-0451-4000-b000-000000000000'
...
class MyDelegate(btle.DefaultDelegate):
    ...
    def bluepy_handler(self): ### TODO: need thorough analysis
        while True:
            if not self.gforce.send_queue.empty():
                cmd = self.gforce.send_queue.get_nowait()
                self.gforce.cmdCharacteristic.write(cmd,withResponse=True) # cmdCharacteristic handle is 30
            try:
                self.gforce.device.waitForNotifications(1)   ## raw data is sent in the form of notification
    ...
    def handleNotification(self, cHandle, data): # called whenever a notification is received from the peripheral
        # check cHandle
...
class GForceProfile():
    def __init__(self):
        ...
        self.send_queue = queue.Queue(maxsize = 20) # 数据上限20，大于20个，阻塞，直到队列中的数据被消掉
...
#        print('mtu:{}'.format(self.mtu))
        ...
        self.notifyCharacteristic = self.getCharacteristic(self.device,NOTIFY_UUID)
...
#        print('mtu:{}'.format(self.mtu))
    ...
    def setNotify(self,Chara,swich):
        ...
        self.device.writeCharacteristic(setup_handle, setup_data, withResponse=False) # write the notification command (self._writeCmd) and then receive a data packet (self._getResp)
    ...
    def scan(self,timeout):
        ...
        for dev in devices:
            for (_, _, value) in dev.getScanData():
                if (value == GUID):
                    gforce_scan.append([i,dev.getValueText(9),dev.addr,dev.rssi,str(dev.connectable)])
                    i+=1        
        return gforce_scan
    ...
    # Set Emg Raw Data Config  TODO: might not work
    ...
    def sendCommand(self,profileCharType,data,hasResponse,cb,timeout):
        ...
        if profileCharType == ProfileCharType.PROF_DATA_CMD:
            if self.cmdCharacteristic == None:
                ...
                if len(data) > self.mtu: # MTU：最大传输单元
                    
                    contentLen = self.mtu - 2
                    packetCount = (len(data)+contentLen-1)//contentLen # // 表示除法向下取整
                    startIndex = 0
                    buf = []
                    for i in range(packetCount-1,0,-1):                        
                        ...
                        self.send_queue.put_nowait(buf) # 由于mtu限制，将data分为多个包，放入队列中
                        buf.clear()
                    # Packet end
                    buf.append(CommandType['CMD_PARTIAL_DATA'])
                    buf.append(0)
                    buf += data[startIndex:]
                    self.send_queue.put_nowait(buf) # data最后，不足一个包的部分
                else:
    ...
    def _refreshTimer(self):
        ...
        if self.timer != None:
            self.timer.cancel()# 如果有timer，取消？
        self.timer = None
        cmdlist = self.cmdMap.values()
        if len(cmdlist) > 0:
            cmdlist = sorted(cmdlist,key=cmp_time) #相当于sorted(cmdlist,key=cmp_time(cmdlist[i])) 以cmp_time(cmdlist[i])为根据进行排序
        ...
        if len(data) >= 2:
            if data[0] == NotifDataType['NTF_PARTIAL_DATA']:
                if self.lastIncompleteNotifPacketId != 0 and self.lastIncompleteNotifPacketId != data[1]+1:
                    print('Error:lastIncompleteNotifPacketId:{0},current packet id:{1}'.format(self.lastIncompleteNotifPacketId,data[1]))
