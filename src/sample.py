...
def ondata(data):
    if len(data) > 0:
        ...
        elif data[0] == DataNotifFlags['DNF_EMG_RAW'] and len(data) == 129:
        ...
        if button == 0:
            ...
            while True:
                ...
                if button == 0:
                    ...
                    ches = GF.device.getCharacteristics(uuid=CMD_UUID)[0]
                    ches.write(b'\xf4\x01\xff\x00\x80\x0c?',True)
                    '''
                    sampRate = eval(
                    ...
                    GF.setEmgRawDataConfig(
                        sampRate, channelMask, dataLen, resolution, cb=set_cmd_cb, timeout=4000)
                    '''
            break